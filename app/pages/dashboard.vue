...
<template>
  <main class="dashboard-page">
    <section v-if="!isPrepared" class="dashboard-empty-state" aria-busy="true">
      <span><LayoutDashboard /></span>
      <p>{{ t("common.loading") }}</p>
    </section>

    <section v-else-if="totalStores === 0" class="dashboard-empty-state">
      <span><LayoutDashboard /></span>
    ...
    <template v-else>
      <DashboardLoadGate
        ...
        :progress="progress"
        @confirm="ensureLoaded"
      />

      <div
        class="dashboard-content"
        :class="{ 'is-gated': !hasLoaded }"
          ...
          <article class="dashboard-panel">
            ...
            <div class="payment-pulse-grid">
              <div>
                ...
                <strong>{{
                  formatMoney(aggregate.payments.transactions.gross)
                }}</strong>
              </div>
              <div>
          ...
          <article class="dashboard-panel">
            ...
            <div class="store-performance-list">
              ...
              >
                ...
                <NuxtLink
                  :to="{ path: '/manager', query: { edit: failure.storeId } }"
                  >{{ t("dashboard.fixAccess") }}</NuxtLink
                >
              </div>
            </div>
          </article>
